...
for item in os.listdir('../food100/output/'): 
	if item.endswith(('.jpg')):
		...
		if (max(width,height)/min(width,height) <= 3 ):
			cropped_image = original_image.crop(((width-smaller_dimension)/2 , (height-smaller_dimension)/2, (width+smaller_dimension)/2 -1 , (height+smaller_dimension)/2-1))
			resized_image = cropped_image.resize((128,128),ANTIALIAS)

			resized_image.save('../food100/output_resized/'+item)
